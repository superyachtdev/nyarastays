...
// ----- Availability / iCal sync (Airbnb → site) ------------------------------
// Each stay maps to the Cloudflare secret holding its Airbnb iCal *export* URL.
const STAY_ICAL_ENV = {
  penthouse: "AIRBNB_PENTHOUSE_ICAL",
};
const AVAIL_TTL_MS = 30 * 60 * 1000; // re-fetch Airbnb at most every 30 min

function pad2(n){ return n < 10 ? "0" + n : "" + n; }
function ymd(y, m, d){ return `${y}-${pad2(m)}-${pad2(d)}`; }

// Parse an iCal date token: DATE "20260115" or DATE-TIME "20260115T110000Z"
...
// Expand every VEVENT into the set of occupied night dates (YYYY-MM-DD).
// iCal DTEND is exclusive (the checkout day), so we block the range [start, end).
function icalToBlockedDates(ics){
  const blocked = new Set();
  if (!ics) return blocked;
  ...
  for (const line of lines){
    ...
    if (line.startsWith("END:VEVENT")){
      if (start){
        const s = Date.UTC(start.y, start.mo - 1, start.d);
        const e = end ? Date.UTC(end.y, end.mo - 1, end.d)
                      : Date.UTC(start.y, start.mo - 1, start.d + 1);
        for (let t = s; t < e; t += 86400000){
          const dt = new Date(t);
          blocked.add(ymd(dt.getUTCFullYear(), dt.getUTCMonth() + 1, dt.getUTCDate()));
        }
      }
      inEvent = false; continue;
    }
    if (!inEvent) continue;
    if (line.startsWith("DTSTART")) start = parseIcalDate(line.slice(line.indexOf(":") + 1));
    else if (line.startsWith("DTEND")) end = parseIcalDate(line.slice(line.indexOf(":") + 1));
  }
  return blocked;
}

async function apiAvailability(request, env, stayKey){
  const envVar = STAY_ICAL_ENV[stayKey];
  if (!envVar) return json({ stay: stayKey, blocked: [], source: "none" });

  const cacheKey = `avail:${stayKey}`;
  // 1. Serve from KV cache if still fresh
  if (env.NYARA_CAL){
    try {
      const cached = await env.NYARA_CAL.get(cacheKey, { type: "json" });
      if (cached && (Date.now() - cached.ts) < AVAIL_TTL_MS){
        return json({ stay: stayKey, blocked: cached.blocked, source: "cache" });
      }
    } catch (_e) { /* ignore */ }
  }

  const icalUrl = env[envVar];
  if (!icalUrl) return json({ stay: stayKey, blocked: [], source: "unconfigured" });

  // 2. Fetch the latest from Airbnb and parse
  let blocked = [];
  let ok = false;
  try {
    const r = await fetch(icalUrl, { headers: { "User-Agent": "NyaraStays/1.0 (+calendar-sync)" } });
    if (r.ok){
      const ics = await r.text();
      blocked = Array.from(icalToBlockedDates(ics)).sort();
      ok = true;
    }
  } catch (_e) { /* network error → fall back to stale cache below */ }

  // 3. On failure, fall back to the last good snapshot so the site never breaks
  if (!ok && env.NYARA_CAL){
    try {
      const stale = await env.NYARA_CAL.get(cacheKey, { type: "json" });
      if (stale && Array.isArray(stale.blocked)) blocked = stale.blocked;
    } catch (_e) { /* ignore */ }
  }

  // 4. Refresh the cache
  if (ok && env.NYARA_CAL){
    try { await env.NYARA_CAL.put(cacheKey, JSON.stringify({ ts: Date.now(), blocked })); }
    catch (_e) { /* ignore */ }
  }

  return json({ stay: stayKey, blocked, source: ok ? "live" : "stale" });
}
...
export default {
  async fetch(request, env) {
    try {
      ...
      const availMatch = p.match(/^\/api\/availability\/([a-z0-9-]+)$/);
      if (availMatch && request.method === "GET") {
        return await apiAvailability(request, env, availMatch[1]);
      }
